...
import { FaFileExcel, FaFileCsv, FaRupeeSign, FaChevronLeft, FaChevronRight } from "react-icons/fa";
...
const DiagnosticsRejectedBooking = () => {
  ...
  const fetchRejectedBookings = async () => {
    try {
      const response = await axios.get("https://api.credenthealth.com/api/admin/rejecteddiagnosticbooking");
      if (response.data && response.data.appointments) {
        setBookings(response.data.appointments);
      }
    } catch (error) {
      console.error("Error fetching rejected diagnostic bookings:", error);
    }
  };
  ...
  const exportToExcel = () => {
    const ws = XLSX.utils.json_to_sheet(
      filteredBookings.map((booking, idx) => ({
        ...
        "User Name": booking.staff_name || "N/A",
    ...
    XLSX.utils.book_append_sheet(wb, ws, "RejectedDiagnosticBookings");
    XLSX.writeFile(wb, "rejected_diagnostic_bookings.xlsx");
  };
  ...
  return (
    <div className="p-4 bg-white rounded shadow">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold text-purple-800">Diagnostics Rejected Bookings</h2>
        <div className="flex gap-2">
          ...
          <CSVLink
            ...
            headers={headers}
            filename="rejected_diagnostic_bookings.csv"
            ...
            {paginatedBookings.length > 0 ? (
              paginatedBookings.map((booking, index) => (
                <tr key={booking.appointmentId} className="hover:bg-gray-50 border-b">
                  ...
                  <td className="p-2 border">
                    {(booking.status && booking.status.toLowerCase() === 'rejected') ? (
                      <span className="px-2 py-1 rounded-full text-xs bg-red-100 text-red-800 capitalize">
                        {booking.status}
                      </span>
                    ) : (
                      <span className="text-gray-600 capitalize">{booking.status || 'N/A'}</span>
                    )}
                  </td>
                </tr>
              ))
