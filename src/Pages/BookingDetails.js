...
const BookingDetails = () => {
  ...
  const {
    ...
    reportFile,
    diagPrescription,
    address // Added address from the booking response
  } = booking;
    ...
    <div className="p-6 bg-white rounded shadow-md max-w-4xl mx-auto">
      ...
      <table className="w-full border rounded mb-6 text-sm">
        <tbody>
          ...
          {/* Address Section - Only for Home Collection */}
          {serviceType === "Home Collection" && address && (
            <>
              <tr className="bg-gray-100">
                <td className="p-2 font-medium border">Address Type</td>
                <td className="p-2 border">{address.addressType}</td>
                <td className="p-2 font-medium border">Postal Code</td>
                <td className="p-2 border">{address.postalCode}</td>
              </tr>
              <tr>
                <td className="p-2 font-medium border">Street</td>
                <td className="p-2 border">{address.street}</td>
                <td className="p-2 font-medium border">City</td>
                <td className="p-2 border">{address.city}</td>
              </tr>
              <tr className="bg-gray-100">
                <td className="p-2 font-medium border">State</td>
                <td className="p-2 border">{address.state}</td>
                <td className="p-2 font-medium border">Country</td>
                <td className="p-2 border">{address.country}</td>
              </tr>
            </>
          )}

          <tr className={serviceType === "Home Collection" ? "" : "bg-gray-100"}>
          ...
          <tr className="bg-gray-100">
            <td className="p-2 font-medium border">Total Price</td>
      ...
      {/* Report & Prescription */}
     {/* Report, Prescription & Received Files */}
<div className="mb-6">
